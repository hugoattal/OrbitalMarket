...
function addComputed(product: IProduct, data: any) {
    ...
    product.computed = {
        ...
        engineVersion: {
            min: (_.first(data.compatibleApps) as string).split(".").map(element => parseInt(element)),
            max: (_.last(data.compatibleApps) as string).split(".").map(element => parseInt(element))
        }
    };
