...
export function addComputed(product: TProductModel) {
    ...
    const score = computeScore(product.review.rating, product.review.count, new Date(product.releaseDate), product.price.value === 0, isBoosted, product.meta?.verificationRatio);
